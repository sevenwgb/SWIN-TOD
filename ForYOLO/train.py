...
Usage - Single-GPU training:
    ...
    python train.py --data AITOD.yaml --weights yolov5m.pt --img 800 --batch-size 1
...
from utils.loss import ComputeLoss,ComputeLoss_instace
...
def train(hyp, opt, device, callbacks):
    ...
    compute_loss_instance = ComputeLoss_instace(model)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                # pred = model(imgs,targets.to(device),compute_loss_instance)  # forward
                if len(pred) == 3:
                    loss, loss_items = compute_loss(pred, targets.to(device))  # loss scaled by batch_size
                if len(pred) == 2:
                    pred_old,pred_new = pred
                    loss, loss_items = compute_loss(pred_old, targets.to(device))  # loss scaled by batch_size
                    loss_new, loss_items_new = compute_loss(pred_new, targets.to(device))
                    loss = 1.0*loss+0*loss_new
                    loss_items = 1.0*loss_items +0*loss_items_new

                # 思路1：根据loss 计算gcn，更新backbonefeature，然后通过head部分结构，根据新得出的pred计算gcn_loss,之后加权到总loss中
                # 思路2：将model分为两个函数，第一部分计算backbone，第二部分计算head。这样就可以根据backbone提取的feature进行两个分支，第一个分支直接进入
                #       head，输出loss，然后将loss和back中提取的feature输入到第二个分支中，计算gcn_loss；之后将两个分支加权起来。
...
def parse_opt(known=False):
    ...
    parser.add_argument("--data", type=str, default=ROOT / "data/AITOD.yaml", help="dataset.yaml path")
