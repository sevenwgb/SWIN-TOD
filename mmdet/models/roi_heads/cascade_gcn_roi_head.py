...
class CascadeGcnRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
        ...
        for i in range(self.num_stages):
            gcn_tmp_model = GCN(num_node_features=256, num_classes=9)
            self.gcn.append(gcn_tmp_model)
        super(CascadeGcnRoIHead, self).__init__(
    ...
    def _bbox_forward_gcn_test(self, stage, x, rois):
        ...
        #以下部分为新增----------------------------------------------------------------
        #【问题】：此处如果使用分类置信度作为权重，相当于只是对roihead的预测结果的进一步增强，那么此分支的结果与roihead中的分类分支是否有区别呢？是否只是前者×系数？
        # 直接对3000个框进行图运算会需要非常大的显存，为了减少显存，此处仅对cls_score中得分前景的框（最多top_M）进行运算
        top_M = 512
        # 构建节点：将bbox_feats由512*256*7*7变为512*256
        ...
        # 将邻接矩阵对角线由0.5变成1
        adjacency_matrix_tensor_fgnode.fill_diagonal_(1)  # 设置对角线值为1
        # 5. 计算节点之间的邻接矩阵 考虑类别信息-连通性矩阵
        ...
        # # 挑选top_M个
        # top_global_avg_pooled_feats = global_avg_pooled_feats[top_indices]
        ...
        data = Data(x=global_avg_pooled_feats, edge_index=edge_index,edge_weight=edge_weight).to(bbox_feats.device)

        model = self.gcn[stage].to(bbox_feats.device)
        # model = GCN(num_node_features=256, num_classes=9).to(bbox_feats.device)
        # 进行GCN前向传播
        top_cls_score_gcn = model(data)

        # 此处将top_indices作为返回值返回到上一层用以进行挑选加权
        bbox_results = dict(
    ...
    # add by sevenwgb 20240128
    def _bbox_forward_gcn_train(self, stage, x, rois, labels):
        ...
        # do not support caffe_c4 model anymore cls_score:512*9  bbox_pred:512*4
        cls_score, bbox_pred = bbox_head(bbox_feats)
        ...
        CEloss_exp = torch.exp(CEloss)
        CEloss_exp_expanded = CEloss_exp.unsqueeze(1)
        adjacency_matrix_tensor_fgnode = CEloss_exp/(CEloss_exp+CEloss_exp_expanded)
        adjacency_matrix_tensor_fgnode = adjacency_matrix_tensor_fgnode.t()
    ...
    def _bbox_forward_train(self, stage, x, sampling_results, gt_bboxes,
        ...
        bbox_results = self._bbox_forward_gcn_train(stage,x,rois,bbox_targets[0])
    ...
    def simple_test(self, x, proposal_list, img_metas, rescale=False):
        ...
        for i in range(self.num_stages):
            # modified by sevenwgb 20240316
            bbox_results = self._bbox_forward_gcn_test(i, x, rois)
            # 获取图卷积运算的节点
            fg_ind = bbox_results['fg_ind']
            cls_score = bbox_results['cls_score']
            cls_score[fg_ind] = 0.9*cls_score[fg_ind]  + 0.1*bbox_results['cls_score_gcn']
